    ...
    private def find_in_path_relative_to_dir(filename, relative_to, check_crystal_path = true)
      if relative_to.is_a?(String)
        ...
        if filename.ends_with?("/*") || (recursive = filename.ends_with?("/**"))
          ...
          # *TODO* *DRY* this hodge podge code up!

          # Check if .onyx file exists.
          relative_filename_cr = relative_filename.ends_with?(".onyx") ? relative_filename : "#{relative_filename}.onyx"
          #p "test onyx file: \"#{relative_filename_cr}\""
          if File.exists?(relative_filename_cr)
            return make_relative_unless_absolute relative_filename_cr
          end

          # Check if .ox file exists.
          relative_filename_cr = relative_filename.ends_with?(".ox") ? relative_filename : "#{relative_filename}.ox"
          #p "test ox file: \"#{relative_filename_cr}\""
          if File.exists?(relative_filename_cr)
            return make_relative_unless_absolute relative_filename_cr
          end
          ...
          #p "test cr file: \"#{relative_filename_cr}\""
          if File.exists?(relative_filename_cr)
            return make_relative_unless_absolute relative_filename_cr
          end
          ...
          if Dir.exists?(relative_filename)
            ...
            absolute_filename = make_relative_unless_absolute("#{relative_filename}/#{basename}.onyx")
            #p "test onyx file: \"#{absolute_filename}\""
            if File.exists?(absolute_filename)
              return absolute_filename
            end

            absolute_filename = make_relative_unless_absolute("#{relative_filename}/#{basename}.ox")
            #p "test ox file: \"#{absolute_filename}\""
            if File.exists?(absolute_filename)
              return absolute_filename
            end

            absolute_filename = make_relative_unless_absolute("#{relative_filename}/#{basename}.cr")
            #p "test cr file: \"#{absolute_filename}\""
            if File.exists?(absolute_filename)
              return absolute_filename
            end
          end
