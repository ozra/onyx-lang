...
class Crystal::Call
  ...
  def lookup_matches_without_splat(arg_types)
    ...
    # elsif   *TODO*  * we need the scopes–list so we can seek for matching VARIABLES
    #                   * we will look for LAMBDA types and
    #                   * types respond_to? call OR functor - decide on one - call might be to clashy with crystal freedom
    #                     * better is ofcourse def ()(bla)  -> "def functor_call_(bla)"
