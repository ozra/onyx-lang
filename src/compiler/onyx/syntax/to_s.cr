...
module Crystal
  ...
  class ASTNode
    ...
    def to_s(io)
      # *TODO* depends of _if_ in crystal and macro - or if Onyx stylizing / _Conversion_ is wanted!
  ...
  class ToOnyxSVisitor < Visitor
    ...
    def visit(node : StringInterpolation)
      # *TODO* optional alternative interpolation delimiters intro
      @str << %(")
    ...
    def visit_interpolation(node)
      node.expressions.each do |exp|
        if exp.is_a?(StringLiteral)
          ...
          # *TODO* optional alternative interpolation delimiters
          startDelim = "\#{"
          endDelim = "}"
    ...
    def visit(node : Def)
      ...
      # *TODO* .org_name (name may be mangled from '-' -> '_')
    ...
    def visit(node : Path)
      node.names.each_with_index do |name, i|
        ...
        # *TODO* *TEMP* see if "." can be unified
        @str << "//" if i > 0 || node.global
        @str << name
      end
    end
