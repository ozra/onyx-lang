...
module Crystal
  ...
  class Nesting
    ...
    property indent
    property name
    property require_end

    def initialize(@kind, @scope, @indent)
      @indent = 0
      @name = ""
      @require_end = false

      if ! Set{  :program, :module, :typedef, :enumdef, :fndef, :block,
                :lambda, :if, :else, :case, :when, :while, :until,
                :rescue, :ensure, :where, :for }.includes? @kind
        raise "Shit went down - don't know about nesting kind '#{@kind.to_s}'"
      end
    end

    def match_end_token(indent, end_token)
      # *TODO* *9*
      # iterate descending
      # if indent > pop the block
      # if indent == and end_token != "" - compare end–token, and possibly "name"
      # stop when indent < passed indent
  ...
  class NestingStack
    ...
    def initialize(scope)
      @stack = [Nesting.new(:program, scope, 0)]
    end

    def push_nesting(kind, scope, indent)
      @stack.push Nesting.new kind, scope, indent
    end
  ...
  class OnyxParser < OnyxLexer
    ...
    def initialize(str, @scope_stack = ScopeStack.new)
      ...
      @nesting_stack = NestingStack.new @scope_stack.last
    ...
    def parse_expressions
      ...
      exp = parse_expression # parse_multi_assign
    ...
    def parse_multi_assign(assignees)
      p "parse_multi_assign - get location"
      location = assignees.location.not_nil!
      #r = assignees.location'!.row
      #r = assignees.location'?.row

      p "parse_multi_assign - check assign, at: '" + @token.type.to_s + "': " + @token.value.to_s
      check :"="
    ...
    def parse_expression_or_lambda_head
      # *TODO*
      parse_expression
    end
    ...
    def parse_idr_or_literal
      ...
      if @token.type == :"{"
        ...
        case tuple_or_hash
        when TupleLiteral
          ary = ArrayLiteral.new(tuple_or_hash.elements, name: idr).at(tuple_or_hash.location)
          return ary
        when HashLiteral
          tuple_or_hash.name = idr
    ...
    def parse_class_def(is_abstract = false, is_struct = false, doc = nil)
      ...
      name = parse_idr allow_type_vars: false
      #skip_space
    ...
    def parse_parenthesized_expression
      ...
      while true
        exps << parse_expression_or_lambda_head
        case @token.type
        when :")"
          @wants_regex = false
          next_token_skip_space
    ...
    def parse_array_literal_or_multi_assign
      p "parse_array_literal_or_multi_assign, at: " + @token.to_s
      node = parse_array_literal

      p "after parse_array_literal, at: " + @token.to_s
      if is_tok? :"="
        p "got '=', at: " + @token.to_s
        if node.of
    ...
    def parse_macro_control(start_line, start_column, macro_state = Token::MacroState.default)
      ...
      when :IDENT
        ...
        when :for
          ...
          while true
            vars << Var.new(check_idr).at(@token.location)
    ...
    def parse_def_helper(is_abstract = false, is_macro_def = false)
      ...
      when :"("
        ...
        while @token.type != :")"
          ...
          elsif is_tok?(:",") || is_tok?(:";") || @token.type == :"NEWLINE"
            next_token_skip_space_or_newline
      ...
      #   p "Got non parens arg_list"
      #   while @token.type != :NEWLINE && @token.type != :";"
      #     extras = parse_arg(arg_list, extra_assigns, false, found_default_value, found_splat)
      #     if !found_default_value && extras.default_value
      #       found_default_value = true
      #     end
      #     if !splat_index && extras.splat
      ...
      if is_macro_def
        ...
        if is_abstract
          ...
          slash_is_regex!
          p "before skip_statement_end, at:" + @token.to_s
          skip_statement_end
          p "after skip_statement_end, at:" + @token.to_s
          ...
          if @token.keyword?(:end)
            ...
            p "before body=parse_expressions, at:" + @token.to_s
            body = parse_expressions
            p "after body=parse_expressions, at:" + @token.to_s
    ...
    def parse_arg(arg_list, extra_assigns, parenthesis, found_default_value, found_splat, allow_restrictions = true)
      ...
      if (allow_restrictions && # && @token.type == :":"
        ...
        is_mod_const, is_mod_mut, type, is_val_const, is_val_mut =
          parse_arg_type()
        p is_mod_const.to_s + ", " + is_mod_mut.to_s + ", " + type.to_s + ", " + is_val_const.to_s + ", " + is_val_mut.to_s
    ...
    def parse_idr_or_global_call
      ...
      when :CONST
        parse_idr_after_colons(location, true, true)
      else
        unexpected_token
      end
    end

    def parse_idr(allow_type_vars = true)
      location = @token.location

      p "parse_idr at: " + @token.to_s
      ...
      check :CONST
      parse_idr_after_colons(location, global, allow_type_vars)
    end

    def parse_idr_after_colons(location, global, allow_type_vars)
      start_line = location.line_number
      start_column = location.column_number

      p "parse_idr_after_colons at: " + @token.to_s
      ...
      p "eof parse_idr_after_colons at: " + @token.to_s
    ...
    def parse_arg_type()
      p "parse_arg_type - check pre mods, at: " + @token.to_s
      ...
      p "parse_arg_type - after parse_simple_type, at: " + @token.to_s
      ...
      p "parse_arg_type - checks done, at: " + @token.to_s
    ...
    def parse_type(allow_primitives)
      ...
      if is_tok?(:"(")
        p "is '('"
        next_token_skip_space_or_newline
        return parse_type_lambda_or_group location, allow_primitives
      else
        p "is not '(': " + @token.to_s
        ret = parse_type_union allow_primitives
        p "parse_type got type from type_union(). At: " + @token.to_s
        return ret
      end
      ...
      # if is_tok?(:"(")
      #   next_token_skip_space_or_newline
      #   if (ret = parse_type_lambda_or_group location, allow_primitives)
      #     success = true
      #   end
      #   return ret
      # end

      # if (ret = parse_type_union allow_primitives)
      #   success = true
      # end
      # return ret

      # ensure
      #   if try_parse && success == false
      #     @token.copy_from(@temp_token)
      #     self.current_pos, @line_number, @column_number = old_pos, old_line, old_column
      #   end
      # end
    ...
    def parse_type_lambda_or_group(location, allow_primitives)
      p "parse_type_lambda_or_group: " + @token.to_s

      if is_tok?(:"->")
        param_type_list = nil
      #if ! is_tok?(:"->")
      else
        param_type_list = splat parse_type_union(allow_primitives) # parse_type_union(allow_primitives)
        #param_type_list = parse_type_union(allow_primitives) # parse_type_union(allow_primitives)
        #param_type_list = [param_type_list] unless param_type_list.is_a?(Array)
        p "initial param_type_list at: " + @token.to_s

        while is_tok?(:",") || is_tok?(:";")
          next_token_skip_space_or_newline

          p "next param_type_list at: " + @token.to_s

          if is_tok?(:"->")
            p "Does this even happen? '->' and then some"
            ...
            type_union = parse_type(allow_primitives)
            if type_union.is_a?(Array)
              param_type_list.concat type_union
            else
              param_type_list << type_union
            end
          end
        end
      end
      ...
      if is_tok? :"->"
        p "was '->': " + @token.to_s
        next_token_skip_space
        p "case on: " + @token.to_s

        case @token.type
        when :",", :")", :"}", :";", :NEWLINE
          p "nil ret: " + @token.to_s
          return_type = nil
        else
          p "ret type at: " + @token.to_s
        ...
        p "2nd ')' check at: " + @token.to_s
        if !lambda_end_paren && is_tok? :")"
          lambda_end_paren = true
          next_token_skip_space
        end
    ...
    def parse_type_union(allow_primitives)
      ...
      p "parse_type_union: " + @token.to_s

      parse_typeunit_with_suffix(types, allow_primitives)

      p "after parse_typeunit_with_suffix: " + @token.to_s
      ...
      if @token.type == :"|"
        ...
        if types.size == 1
          p "after '|' loop, one type: " + types.first.to_s + " at: " + @token.to_s
          return types.first
        else
          p "after '|' loop, got union: " + types.to_s + " at: " + @token.to_s
          return Union.new types
        end

      elsif types.size == 1
        p "after '|' CHECK, got single type: " + types.first.to_s + " at: " + @token.to_s
        return types.first

      else
        p "!!? after '|' CHECK, got multiple types (!?): " + types.to_s + " at: " + @token.to_s
        return types
      end
    end

    def parse_typeunit_with_suffix(types, allow_primitives)
      p "parse_typeunit_with_suffix at: " + @token.to_s
      # *TODO* *TYPE*
      if @token.keyword? :SelfType
        p "was Self at: " + @token.to_s
        type = Self.new
        next_token_skip_space
      else
        case @token.type
        ...
        when :"("
          ...
          p "parse_typeunit_with_suffix, before parse_simple_type: " + @token.to_s
          type = parse_simple_type
          p "parse_typeunit_with_suffix, after parse_simple_type: " + @token.to_s
    ...
    def parse_simple_type
      if @token.keyword?(:typeof)
        ...
        p "parse_simple_type, before parse_idr: " + @token.to_s
        type = parse_idr
        p "parse_simple_type, after parse_idr: " + @token.to_s
      end
      skip_space
      p "parse_simple_type, after skip_space: " + @token.to_s
      type
    end
    ...
    def parse_struct_or_union_fields(exps)
      ...
      while @token.type == :","
        next_token_skip_space_or_newline
        args << Arg.new(check_idr).at(@token.location)
        next_token_skip_space_or_newline
      end
    ...
    def check_idr(value)
      raise "expecting idrifier '#{value}', not '#{@token.to_s}'", @token unless @token.keyword?(value)
    end
