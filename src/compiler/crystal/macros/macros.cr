class Crystal::Program
  ...
  def expand_macro(a_macro : Macro, call : Call, scope : Type, path_lookup : Type? = nil)
    interpreter = MacroInterpreter.new self, scope, path_lookup || scope, a_macro, call, is_onyx: a_macro.is_onyx
    a_macro.body.accept interpreter
    interpreter.to_s
  end

  def expand_macro(node : ASTNode, scope : Type, path_lookup : Type? = nil, free_vars = nil)
    interpreter = MacroInterpreter.new self, scope, path_lookup || scope, node.location, lang: node.is_onyx ? :onyx : :crystal
