...
class Crystal::Type
  ...
  def lookup_type(node : ASTNode, self_type = self.instance_type, allow_typeof = true, free_vars : Hash(String, TypeVar)? = nil) : Type
    _dbg "def lookup_type(node : ASTNode, self_type = self.instance_type, allow_typeof = true, free_vars : Hash(String, TypeVar)? = nil) : Type"
  ...
  def lookup_type_var(node : Path, free_vars : Hash(String, TypeVar)? = nil) : Type | ASTNode
    _dbg "def lookup_type_var(node : Path, free_vars : Hash(String, TypeVar)? = nil) : Type | ASTNode"
  ...
  def lookup_type_var?(node : Path, free_vars : Hash(String, TypeVar)? = nil, raise = false) : Type | ASTNode | Nil
    _dbg "def lookup_type_var?(node : Path, free_vars : Hash(String, TypeVar)? = nil, raise = false) : Type | ASTNode | Nil"
  ...
  struct TypeLookup
    def initialize(@root : Type, @self_type : Type, @raise : Bool, @allow_typeof : Bool, @free_vars : Hash(String, TypeVar)? = nil)
      _dbg "def initialize(@root : Type, @self_type : Type, @raise : Bool, @allow_typeof : Bool, @free_vars : Hash(String, TypeVar)? = nil)"
      @in_generic_args = 0
    ...
    def lookup(node : Path)
      _dbg "TypeLookup#lookup(node : Path)"
    ...
    def lookup(node : Metaclass)
      _dbg "def lookup(node : Metaclass)"
    ...
    def lookup(node : ProcNotation)
      _dbg "def lookup(node : ProcNotation)"
      types = [] of Type
    ...
    def check_cant_infer_generic_type_parameter(scope, node)
      _dbg "def check_cant_infer_generic_type_parameter(scope, node)"
