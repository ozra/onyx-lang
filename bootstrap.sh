...
echo "Gets Crystal 0.9.1"

# curl can't handle the redirect to data-store!
wget 'https://github.com/manastech/crystal/releases/download/0.9.1/crystal-0.9.1-1-linux-x86_64.tar.gz' -O - | tar zx

echo "Installs Crystal 0.9.1 at /opt/crystal/"

sudo mkdir -p /opt
sudo cp -a crystal-0.9.1-1/ /opt/crystal
sudo ln -fs /opt/crystal/bin/crystal /usr/local/bin/crystal

rm -rf crystal-0.9.1-1
